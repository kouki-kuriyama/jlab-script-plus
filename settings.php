...
/*
	...
	　すべてのファイルをアップロード後、ブラウザから index.php にアクセスします。
...
//同時にアップロード許可枚数
//　複数枚の同時アップロードを許可する場合は2以上を設定します。
//　許可しない（単一アップロードのみを許可する）場合は1を設定します。
//　ドラッグアンドドロップアップロードが無効及び使用できない場合は自動的に単一アップロードのみを許可します。
$MultiUploadLimit = 10;
...
/*
	...
	アップロード後、ブラウザから index.php にアクセスしてアップローダーの設定を完了してください。
...
*/
	...
	if( !file_exists("./static/settings.dat") ){
		...
		$AddSettingsData .= $MultiUploadLimit;
		...
		AddLogData("settings.datを保存しました","Info");
		AddLogData("稼働を開始しました","Info");
		echo "［ｉ］settings.dat を保存しました。\n";
		echo "　　　ページをリロードしてアップローダーが正常に表示されるかご確認ください。\n\n";
		exit;
